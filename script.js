// --- Configuration Rules & Variables ---
        ...
        function releaseWakeLock() {
         ...
         // --- 📌 Pin both Playlist + Active Sentence toolbars ---
function updatePinnedToolbarsPosition() {
    ...
    // If the sentence card itself is pinned, its sticky bottom is the anchor.
    // Otherwise the sticky video is the anchor.
    if (sentenceBoxPinned && sentenceDisplayCard) {
        const cardRect = sentenceDisplayCard.getBoundingClientRect();
        anchorBottom = cardRect.bottom;
    } else if (videoVisible) {
        const videoRect = videoContainer.getBoundingClientRect();
        anchorBottom = videoRect.bottom;
    }

    const gap = 8;
    const firstTop = Math.max(0, Math.round(anchorBottom + gap));
    const secondTop = firstTop + playlistToolbar.getBoundingClientRect().height + gap;
...
function setToolbarsPinned(pinned) {
    ...
    if (actPinToolbarsBtn) {
        ...
        actPinToolbarsBtn.title = toolbarsPinned
            ...
            : 'Pin Playlist Toolbar and Active Sentence Toolbar';
