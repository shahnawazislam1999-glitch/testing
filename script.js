// --- Configuration Rules & Variables ---
        ...
        function releaseWakeLock() {
         ...
         function updatePinnedToolbarsPosition() {
             ...
             // The pinned toolbar pair follows the pinned sentence card when it
             // is enabled. Otherwise it follows the sticky video.
             let anchorBottom = 0;
             if (sentenceBoxPinned && sentenceDisplayCard) {
                 anchorBottom = sentenceDisplayCard.getBoundingClientRect().bottom;
             } else if (videoVisible) {
                 anchorBottom = videoContainer.getBoundingClientRect().bottom;
             }

             const firstTop = Math.max(0, Math.round(anchorBottom + 8));
